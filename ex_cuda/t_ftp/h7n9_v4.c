 ...
 * compilation:  gcc ./h7n9_v4.c -lm
 * usage:        ./a.out  >  ./h7n9_v4.0.dat 
 *               tail -499 ./h7n9_v4.0.dat  >  ./h7n9_v4.1.dat
 *               gnuplot  "./h7n9_v4.gnuplot"
 *               gs ./h7n9_v4.eps                       
 */
        ...
        for (j=0; j<NMB_CELL_TYPES; j++) {
            if (j == H7N9_TISSUE_TARGET) {
               if ((time % H7N9_TRANSMISSION_TIME) == 0) {
                  ...
                  * release their load of new mature H7N9 virus particles in
                  * order to infect new cells of the target tissue;
                  */
               ...
               */
               if (time >= HOURS_2ACTIVATE_IMMUNE_SYSTEM) {
                  nmb_infected_cells[j] *= exp(-0.000010969388 * time * time);  
               }
            } else {

              /*
               * just dummy actions of potential interest in future;
               */
