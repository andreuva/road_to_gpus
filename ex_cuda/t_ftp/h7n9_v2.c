 ...
 *               the fraction of lung/bronchi/bronchioles cells within all
 *               types of cells forming the body; in case of infection, the 
 *               progress of the virus is modeled in terms of numbers of 
 *               cells being infected over time;
 *               n.b. here all defense mechanisms of the immune system are 
 *                    considered to be perfectly absent !
 *               n.b.2 in this variant a generalization is introduced that
 *                     basically allows consideration of parallel infections
 *                     of various types of other tissues, despite the focus 
 *                     on an explicitly selected target tissue most relevant 
 *                     to the case of H7N9 infection;
 * result:       when struck by H7N9 the progress is deterministic with
 *               the virus invading host cells in an exponential manner 
 *               until the host eventually dies 126 hours post infection;
 * compilation:  gcc ./h7n9_v2.c 
 * usage:        ./a.out  >  ./h7n9_v2.0.dat 
 *               tail -130 ./h7n9_v2.0.dat | head -126  >  ./h7n9_v2.1.dat
 *               gnuplot  "./h7n9_v2.gnuplot"
 *               gs ./h7n9_v2.eps                       
 */
...
#define NMB_INITIAL_INFECTION_ATTEMPTS 10
...
#define NMB_TIME_STEPS_2STUDY 336
...
int main()
{
    ...
    */
    sick_cell_type_count = (float *) malloc(NMB_CELL_TYPES * sizeof(float));
    nmb_infected_cells = (float *) malloc(NMB_CELL_TYPES * sizeof(float));
    ...
    */
    ...
    free(nmb_infected_cells);
    free(sick_cell_type_count);
                                ...
                                float *nmb_infected_cells)          
{
    ...
    */
    for (i=1; i<NMB_TIME_STEPS_2STUDY; i++) {
        ...
        * they are directly implicated in H7N9 infection;
        */
        for (j=0; j<NMB_CELL_TYPES; j++) {
            if (j == H7N9_TISSUE_TARGET) {
               if ((time % H7N9_TRANSMISSION_TIME) == 0) {
                  ...
                  */
                  sick_cell_type_count[j] -= nmb_infected_cells[j]; 
                  nmb_infected_cells[j] *= (float) H7N9_REPRODUCTION_NMB;
               }
            } else {
                 ...
                 * cross-infections;
                 */
                 sick_cell_type_count[j] += 0.0;
                 nmb_infected_cells[j] *= 1.0;
            }
        }
        printf("%8d hours post infection, nmb infected cells %14.6e\n", time, nmb_infected_cells[H7N9_TISSUE_TARGET]);
