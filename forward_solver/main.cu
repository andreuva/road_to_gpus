...
const float a = 1e-4;                           /* # dumping Voigt profile a=gam/(2^1/2*sig) */
const float r = 1e-4;                           /* # line strength XCI/XLI */
const float eps = 1e-3;                         /* # Phot. dest. probability (LTE=1,NLTE=1e-4) */
const float dep_col = 1;                        /* # Depolirarization colisions (delta) */
const float Hd = 0.5;                           /* # Hanle depolarization factor [1/5, 1] */
...
__global__ void psi_calc(double deltaum[], double deltaup[], \
...
__global__ void RTE_SC_solve(double II[][nw][qnd], double QQ[][nw][qnd], double SI[nz][nw][qnd],\
